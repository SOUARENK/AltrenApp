...
def fetch_ics_events_structured() -> list[dict]:
    """
    Récupère les événements de tous les calendriers ICS connectés
    et les retourne sous forme de dicts compatibles AgendaEvent.
    """
    if httpx is None or Calendar is None:
        return []
    conns = _get_connections("ics_calendar")
    ...
    for conn in conns:
        url = conn["config"]["url"]
        source = url.split("/")[2] if url.count("/") >= 2 else "ics"
        try:
            ...
            for component in cal.walk():
                ...
                uid = str(component.get("uid", "")) or f"ics-{len(events)}"
                ...
                events.append({
                    "id": uid,
                    "title": summary,
                    "start": _iso(dtstart) or "",
                    "end": _iso(dtend),
                    "source": source,
                    "location": str(location) if location and str(location) not in ("", "None") else None,
                    "description": str(description)[:300] if description and str(description) not in ("", "None") else None,
                })
        except Exception as exc:
            logger.warning("Impossible de récupérer le calendrier ICS %s : %s", url[:60], exc)
    return events
